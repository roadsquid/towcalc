...
function updateCalculator() {
  ...
  const pinPercent = num("pinPercent");
  ...
  const estimatedPinWeight = trailerWeight * (pinPercent / 100);
  const payloadLeftForPin = adjustedPayloadLimit - hitchWeight - passengerWeight - cargoWeight;
  const payloadDifference = payloadLeftForPin - estimatedPinWeight;
  const towDifference = adjustedTowLimit - trailerWeight;

  byId("pinWeightResult").textContent = formatLbs(estimatedPinWeight);
  byId("payloadLeftResult").textContent = formatLbs(payloadLeftForPin);
  ...
  if (payloadDifference < 0 || towDifference < 0) {
    ...
    if (payloadDifference < 0 && towDifference < 0) {
      ...
      limiterText = "Payload is the main problem. Your estimated pin weight is using more than your adjusted payload allows.";
    } else {
    ...
    if (payloadDifference <= towDifference) {
      ...
      limiterText = "Payload is getting tight. A little more cargo, passengers, or real-world pin weight could push this setup over the line.";
    } else {
